...
export class DispatchFromLevelModuleService {
  ...
  async dispatchFromLevel(dispatchFromLevelDto) {
    ...
    if (!levelState) {
      userState[level.name.en_US] = {}
    }
    levelState = userState[level.name.en_US]
    ...
    levelState['level'] = headless
    levelState['updatedAt'] = new Date()
  ...
  clearOldState() {
    ...
    for (let jwtToken of Object.keys(state)) {
      for (let levelName of Object.keys(state[jwtToken])) {
        const dateDiffInMilliSeconds = now - state[jwtToken][levelName]
